...
# Dictionnaire des types MIME basés sur l'extension du fichier
mime_types_mapping = {
    "pdf": "application/pdf",
    "txt": "text/plain",
    "html": "text/html",
...
def generate_filename(original_name, directory):
    ...
    return os.path.join(directory, f"{timestamp}_{name}{ext}")
...
def generate_document_content(uploaded_file, question):
    ...
    sample_doc = gen_ai.upload_file(io.BytesIO(doc_bytes), mime_type=mime_type)
...
def main():
    ...
    # Section : Analyse de document
        ...
        uploaded_file = st.file_uploader(
            "Chargez un fichier (formats acceptés : PDF, TXT, HTML, Markdown, CSV, XML, CSS, RTF, JavaScript, Python).",
        ...
        if st.button("Analyser le document"):
            if uploaded_file and user_question:
                try:
                    ...
                    document_path = generate_filename(uploaded_file.name, DOC_DIR)
